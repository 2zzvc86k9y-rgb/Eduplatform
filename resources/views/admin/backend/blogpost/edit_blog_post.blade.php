        ...
        <div class="ps-3">
            <nav aria-label="Fil d'Ariane">
                <ol class="p-0 mb-0 breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Modifier un article de blog</li>
                </ol>
            </nav>
        </div>
    </div>
        ...
        <div class="mx-auto col-xl-10">
            <div class="card shadow-sm">
                <div class="p-4 card-body">
                    <h5 class="mb-4">Modifier un article de blog</h5>
                    <form id="myForm" action="{{ route('update.blog.post') }}" method="POST" class="row g-3" enctype="multipart/form-data">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" class="form-control @error('post_title') is-invalid @enderror" id="post_title" name="post_title"
                                value="{{ old('post_title', $post->post_title) }}" required aria-describedby="post_title_error">
                            @error('post_title')
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select name="blogcat_id" id="blogcat_id" class="mb-3 form-select @error('blogcat_id') is-invalid @enderror"
                                ...
                                @foreach ($blogCat as $item)
                                    <option value="{{ $item->id }}" {{ $item->id == old('blogcat_id', $post->blogcat_id) ? 'selected' : '' }}>
                                        {{ $item->category_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-md-12">
                            ...
                            <textarea class="form-control @error('long_description') is-invalid @enderror" name="long_description" id="summernote"
                                placeholder="Écrivez la description de l'article..." rows="3">{!! old('long_description', $post->long_description) !!}</textarea>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" name="post_tags" id="post_tags" class="form-control @error('post_tags') is-invalid @enderror"
                                data-role="tagsinput" value="{{ old('post_tags', $post->post_tags) }}">
                            @error('post_tags')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input class="form-control @error('post_image') is-invalid @enderror" type="file" id="post_image" name="post_image"
                                aria-describedby="post_image_error">
                            @error('post_image')
                        ...
                        <div class="col-md-6">
                            <img id="showImage" src="{{ $post->post_image ? asset($post->post_image) : url('upload/noimage.jpg') }}"
                                alt="Aperçu de l'image de l'article" class="p-1 mt-2 rounded bg-light border"
                                style="width: 100px; height: 100px; object-fit: cover;">
                        </div>

                        <div class="col-md-12">
                            <div class="gap-3 d-md-flex d-grid align-items-center">
                                <button type="submit" class="px-4 btn btn-primary" aria-label="Mettre à jour l'article">Mettre à jour</button>
                            </div>
                        </div>
                    </form>
                </div>
