            ...
            <div class="col-md-10">
                <div class="quiz-oc-card">
                    <div class="quiz-oc-header text-center mb-4">
                        <h2 class="quiz-oc-title">{{ $quiz->title }}</h2>
                        <div class="quiz-oc-meta">
                            <span class="quiz-oc-meta-item"><i class="la la-clock-o"></i> {{ $quiz->duration }} min</span>
                            <span class="quiz-oc-meta-item"><i class="la la-trophy"></i> Score min : {{ $quiz->passing_score }}%</span>
                            <span class="quiz-oc-meta-item"><i class="la la-star"></i> {{ $quiz->total_points }} pts</span>
                        </div>
                    ...
                    <div class="quiz-oc-progress-bar mb-4">
                        <div class="quiz-oc-progress" style="width: 0%" id="quizProgressBar"></div>
                    </div>

                    <form id="quiz-form" action="{{ route('submit.quiz', [$course->id, $quiz->id]) }}" method="POST">
                        @csrf
                        @foreach($quiz->questions as $index => $question)
                        <div class="quiz-oc-question mb-5">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <span class="quiz-oc-step">Question {{ $index + 1 }} / {{ count($quiz->questions) }}</span>
                            ...
                            @else
                                <input type="text" class="quiz-oc-input" name="answers[{{ $question->id }}]" required>
                            @endif
                        </div>
                        @endforeach
                        ...
                        <div class="quiz-oc-actions mt-4">
                            ...
                            <a href="{{ route('course.view', $course->id) }}" class="quiz-oc-btn quiz-oc-btn-secondary">Annuler</a>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        // Affiche la progression (100% si tout affiché d'un coup)
