        ...
        <div class="container">
            <div class="flex-wrap breadcrumb-content d-flex align-items-center justify-content-between">
                <div class="section-heading">
                    <h2 class="text-white section__title">Connexion</h2>
                </div>
            ...
            <div class="row">
                <div class="mx-auto col-lg-7">
                    <div class="card card-item">
                        <div class="card-body">
                            <h3 class="pb-4 text-center card-title fs-24 lh-35">Connectez-vous à votre compte !</h3>
                            <div class="section-block"></div>
                            <form method="POST" action="{{ route('login') }}" class="pt-4 shadow-sm">
                                @csrf
                                <div class="input-box">
                                    <label class="label-text">Email</label>
                                    <div class="form-group">
                                        <input class="form-control form--control" type="email" name="email"
                                            id="email" placeholder="Email ou Nom d'utilisateur" value="{{ old('email') }}"
                                            required autofocus>
                                    ...
                                    <div class="pb-4 d-flex align-items-center justify-content-between">
                                        <div class="custom-control custom-checkbox fs-15">
                                            ...
                                            <label class="custom-control-label custom--control-label"
                                                for="rememberMeCheckbox">Se souvenir de moi</label>
                                        </div><!-- end custom-control -->
                                        @if (Route::has('password.request'))
                                            <a href="{{ route('password.request') }}" class="btn-text">Mot de passe oublié ?</a>
                                        @endif
                                    </div>
                                    <button class="btn theme-btn" type="submit">Se connecter <i
                                            class="ml-1 la la-arrow-right icon"></i></button>
                                    <p class="pt-2 fs-14">Vous n'avez pas de compte ? <a href="{{ route('register') }}"
                                            class="text-color hover-underline">S'inscrire</a></p>
                                </div><!-- end btn-box -->
                            </form>
                        </div><!-- end card-body -->
                    </div><!-- end card -->
